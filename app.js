...
/* V10 SAFARI STORAGE FALLBACK */
(function(){
  try { const k="__level2031_storage_test"; localStorage.setItem(k,"1"); localStorage.removeItem(k); }
  catch(e) {
    const mem={};
    window.__level2031MemoryStorage=mem;
    window.localStorage={
...
function more(){return `<div class="section"><h1>More</h1><p>Settings, backup/restore, notifications and future Level 2031 systems.</p><div class="grid"><button class="card action" onclick="toggleTheme()">${s.theme==='dark'?'☀️ Light mode':'🌙 Dark mode'}</button><button class="card action" onclick="exportBackup()">💾 Export backup</button><button class="card action" onclick="resetApp()">↻ Reset app</button></div></div>`}
function world(){
  if(window.Level2031RPG && window.Level2031RPG.getMarkup){
    return window.Level2031RPG.getMarkup();
  }
  return `<section class="rpg-world"><div class="rpg-topbar"><div><div class="rpg-kicker">LEVEL 2031 • WORLD</div><h2>🌎 World</h2><p>Loading your future world…</p></div></div></section>`;
}
const home=homeView;
function toggleTheme(){s.theme=s.theme==='light'?'dark':'light';save();render(currentView)}
let currentView='home';
function render(view='home'){currentView=view;ensureDaily();recalc();document.body.className=s.theme==='light'?'light':'';const views={home:homeView,missions,stats,goals,finance,hvac,trading,fitness,faith,development,relationships,travel,vehicles,achievements,world,log,more};let content;
try{content=(typeof views[view]==='function'?views[view]:homeView)();}
catch(err){
  console.error('Level 2031 render error',err);
  content=`<section class="section"><h1>Level 2031</h1><p>The selected screen hit an error. Returning to Home…</p><button class="btn" onclick="render('home')">Return Home</button><pre style="white-space:pre-wrap;opacity:.65;font-size:11px">${esc(err&&err.stack?err.stack:err)}</pre></section>`;
}const nav=[['home','⌂','Home'],['missions','⚔','Missions'],['stats','◈','Stats'],['log','✦','Life Log'],['world','🌎','World'],['more','☰','More']];document.getElementById('app').innerHTML=`<div class="shell"><header class="top"><div><div class="brand">LEVEL 2031</div><div class="level">${today()} • ${localTime()} • LEVEL ${levelFor(s.xp)}</div></div><button class="themeBtn" onclick="toggleTheme()">${s.theme==='dark'?'☼':'☾'}</button></header><main class="content">${content}</main><div class="toast"></div><nav class="nav"><div class="navin">${nav.map(([v,i,t])=>`<button class="${view===v?'active':''}" onclick="render('${v}')"><span>${i}</span>${t}</button>`).join('')}</div></nav></div>`}
ensureDaily();recalc();save();render('home');
/* V10 WORLD EVENT DELEGATION */
document.addEventListener('click',function(e){
  const room=e.target.closest && e.target.closest('[data-rpg-room]');
  if(room && window.Level2031RPG){
    const r=window.Level2031RPG.state;
    r.world.activeRoom=room.dataset.rpgRoom;
    r.world.discovered=[...new Set([...(r.world.discovered||[]),r.world.activeRoom])];
    localStorage.setItem('level2031_rpg_v9',JSON.stringify(r));
    render('world');
    return;
  }
  if(e.target.id==='rpgCustomize' && window.Level2031RPG){
    const r=window.Level2031RPG.state;
    const outfit=prompt('Outfit: Everyday, Work, Gym, Formal, Travel',r.character.outfit);
    if(outfit) r.character.outfit=outfit;
    const build=prompt('Build: Athletic, Lean, Strong, Casual',r.character.build);
    if(build) r.character.build=build;
    localStorage.setItem('level2031_rpg_v9',JSON.stringify(r));
    render('world');
  }
});
...
(function initVisualRPG(){
  ...
  // V10: the main app render() owns the World screen. Do not auto-mount a second copy.
})();
...
(function hardenWorldScreen(){
  ...
  // V10: the World tab renders through render('world'); avoid injecting a duplicate host at startup.
})();
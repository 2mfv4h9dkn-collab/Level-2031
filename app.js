...
function render(view='home'){currentView=view;ensureDaily();recalc();document.body.className=s.theme==='light'?'light':'';const views={home,missions,stats,goals,finance,hvac,trading,fitness,faith,development,relationships,travel,vehicles,achievements,log,world,more};const content=(views[view]||home)();const nav=[['home','⌂','Home'],['missions','⚔','Missions'],['stats','◈','Stats'],['log','✦','Life Log'],['more','☰','More']];document.getElementById('app').innerHTML=`<div class="shell"><header class="top"><div><div class="brand">LEVEL 2031</div><div class="level">${today()} • ${localTime()} • LEVEL ${levelFor(s.xp)}</div></div><button class="themeBtn" onclick="toggleTheme()">${s.theme==='dark'?'☼':'☾'}</button></header><main class="content">${content}</main><div class="toast"></div><nav class="nav"><div class="navin">${nav.map(([v,i,t])=>`<button class="${view===v?'active':''}" onclick="render('${v}')"><span>${i}</span>${t}</button>`).join('')}</div></nav></div>`}
